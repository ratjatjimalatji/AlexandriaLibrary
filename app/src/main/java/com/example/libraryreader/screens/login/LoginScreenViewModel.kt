...
auth.signInWithEmailAndPassword(email, password)
    .addOnCompleteListener() { task ->
        if( task.isSuccessful){
            Log.d("Firebase", "signInWithEmailAndPassword: Login SUCCESS ${task.result.toString()}")
            //TODO("Take user to home page")
home()
        }else{
            Log.d("Firebase", "signInWithEmailAndPassword: Login FAILED ${task.result.toString()}")
        }}
        }catch(ex: Exception){
